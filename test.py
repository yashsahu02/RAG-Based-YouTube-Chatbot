...
if button:
    if not video_url:
        ...
        try:
            ...
            # The original code only wrote the docs, but didn't process them.
            # This is where the core fix is applied.

            # Convert loaded docs to a single transcript string
            transcript = " ".join([doc.text for doc in transcript_list])
            
            # Update session state with new video data
            st.session_state['video_id'] = video_url.split("v=")[-1].split("&")[0]
            st.session_state['transcript'] = transcript
            st.session_state['transcript_list'] = transcript_list # Storing docs is better than a list of text
            
            st.subheader("Transcript ->")
            st.write(st.session_state['transcript'])
            st.success("Transcript loaded successfully! You can now ask questions.")

            # --- RAG Pipeline Re-initialization ---
            # This section was previously commented out or outside the `if button:` block.
            # It must be inside here to work with the new transcript.
